...
| 网页版（免费） | `chat.deepseek.com` 及官方页面引用的 DeepSeek 子域、CDN、验证码、搜索与分享服务 | 用户首次切换到网页版后；页面隐藏时不强制断网或刷新 | 官方网页登录态、消息、上下文、附件及用户在官方页面触发的操作；客户端不附加 Harness Key |
...
DeepSeek 网页登录、聊天和充值可能访问官方页面引用的验证码、CDN、支付、搜索、朗读或分享服务；这些由官方网页随时决定，不能仅凭此表当作完整防火墙策略。外部网页导航会交给系统浏览器，DeepSeek 官方域内导航留在隔离容器。点击广告会访问所展示的外部目标，例如 DeepSeek 官网、400 电话服务或阿里云市场；不点击时客户端不会为了预览而抓取这些目标网页。
