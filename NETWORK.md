...
适用版本：0.3.1。以下是客户端内置路径，不是模型工具或官方网页所有可能请求的白名单。
...
| 网页版（免费） | `chat.deepseek.com` 及官方页面引用的 DeepSeek 子域、CDN、验证码、搜索与分享服务 | 用户首次切换到网页版后；页面隐藏时不强制断网或刷新 | 官方网页登录态、消息、上下文、附件及用户在官方页面触发的操作；客户端不附加 Harness Key；User-Agent 保留真实系统/Chromium 版本但去掉 Electron 产品标识 |
...
DeepSeek 网页登录、聊天和充值可能访问官方页面引用的验证码、CDN、支付、搜索、朗读或分享服务；这些由官方网页随时决定，不能仅凭此表当作完整防火墙策略。外部网页导航会交给系统浏览器，DeepSeek 官方域内导航留在隔离容器。客户端不篡改官方请求正文、验证码或 Cookie；去掉 Electron User-Agent 产品标识仅用于让底层 Chromium 按普通兼容页面加载。官方服务仍可按账号、IP、频率或策略拒绝访问，客户端不会自动连续重试。点击广告会访问所展示的外部目标，例如 DeepSeek 官网、400 电话服务或阿里云市场；不点击时客户端不会为了预览而抓取这些目标网页。
